...
import { ITranslations, IBlogDataType } from "../../types/Common";
...
const BlogSection = ({ locale = "en" }: BlogSectionProps) => {
  ...
  const blogData: IBlogDataType[] = getBlogTranslations(locale);
  ...
  return (
    <>
      <div className="bg-white max-w-[1280px] sm:mx-auto mx-4">
        ...
        {/* Gallery Grid */}
        <div className="grid grid-cols-2 sm:grid-cols-5 lg:sm:grid-cols-5 md:sm:grid-cols-3 justify-center items-center gap-0 max-w-screen-xl sm:mx-auto">
...
<div className="my-12 max-w-[1280px] mx-4 sm:mx-auto overflow-hidden">
  <h2
  ...
  >
    {filteredBlogs.map(blog => (
      <SwiperSlide key={blog.url} className="h-auto pr-[10px] pl-[10px]">
        <div className="bg-white rounded-[23px] shadow-[0px_0px_2px_1px_#00000040] flex flex-col h-full mx-0 mb-[100px] mt-[5px]">
          <div
            className="w-full h-[160px] sm:h-[210px] bg-cover bg-center rounded-t-[23px] border-b border-gray-300"
            style={{ backgroundImage: `url(${blog.feature_image})` }}
          ></div>
          <div className="w-full px-4 py-6 sm:py-10 flex flex-col justify-center gap-2 sm:gap-5">
            <div className="flex items-start space-x-2">
              <p className="text-[16px] text-[#666666BF] whitespace-nowrap">{blogs.blogCard.company}</p>
              <ul className="list-disc pl-5 space-y-2 text-[16px] text-[#666666BF] marker:text-[#025C7A]">
                <li>{blogs.blogCard.location}</li>
              </ul>
            </div>

            <p className="text-[18px] sm:text-[22px] text-[#04000B] line-clamp-2 font-semibold hover:text-[#6E9753]">
