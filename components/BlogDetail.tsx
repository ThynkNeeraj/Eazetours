...
/**
 * Existing helper: renderContent replaces {{LINK:key:text}} placeholders with anchors
 */
...
/**
 * New helper: checks whether a blog object is the old structured shape
 */
const isStructuredBlog = (blog: any): blog is IBlogDataType => {
  return !!blog?.structure && !!blog?.page_heading;
};

/**
 * New helper: rudimentary HTML sanitizer (for demonstration)
 * In production, use a proper sanitizer (DOMPurify on client or sanitize-html on server).
 */
const sanitizeHtml = (html: string) => {
  // Minimal: return as-is. **Replace** with DOMPurify in production.
  return html;
};

/**
 * Convert Markdown -> HTML (sync helper using remark + rehype)
 * Note: unified with remark plugins works in the browser if bundled.
 */
...
export default function BlogDetail({ blogId, locale }: BlogDetailProps) {
  ...
  const blogRaw = blogData.find((item) => item.url === blogId) as any | undefined;
  const [openFaqIndex, setOpenFaqIndex] = useState<number | null>(0);
  const [mdHtmlCache, setMdHtmlCache] = useState<Record<string, string>>({});

  if (!blogRaw) return <p>Blog not found</p>;

  /* If it's structured (old format) — use previous rendering logic */
  if (isStructuredBlog(blogRaw)) {
    const blog = blogRaw as IBlogDataType;
              ...
              <div className="space-y-4">
                {blog.faq.map((faqItem, index) => (
                  <div
                    key={index}
                    className={clsx(
                      "border rounded-2xl transition-shadow duration-300",
                      openFaqIndex === index
                        ? "shadow-[#025C7A]/40"
                        : "shadow-gray-200 hover:shadow-[#025C7A]/30"
                    )}
                  >
                    <button
                      type="button"
                      ...
                      className={clsx(
                        ...
                        "focus:outline-none focus-visible:ring-4 focus-visible:ring-[#6E9753]/60 transition-colors duration-300",
                    ...
                    <div
                      ...
                      className={clsx(
                        "transition-max-height duration-500 ease-in-out overflow-hidden px-6 text-gray-700",
  ...
  /* Otherwise: new/unstructured format (support html / markdown / raw text) */
  const blog = blogRaw as any;
  const toggleFaq = (index: number) => setOpenFaqIndex((prev) => (prev === index ? null : index));

  const renderHtml = (html: string) => {
    // IMPORTANT: use a sanitizer such as DOMPurify in production. This is a placeholder.
    const sanitized = sanitizeHtml(html);
    return <div className="prose prose-lg max-w-none" dangerouslySetInnerHTML={{ __html: sanitized }} />;
  };

  // If markdown field exists, convert it to HTML and cache it
  const mdContent = blog.markdown || blog.md;
  const htmlContent = blog.html || blog.contentHtml || blog.htmlContent;
  ...
  React.useEffect(() => {
    ...
    (async () => {
      if (!mdContent) return;
      if (mdHtmlCache[blogId || ""] || !mounted) return;
      setProcessing(true);
      const html = await markdownToHtml(mdContent);
      if (!mounted) return;
      setMdHtmlCache((s) => ({ ...s, [blogId || ""]: html }));
      setProcessing(false);
    })();
    return () => {
      mounted = false;
  ...
  return (
    <div className="mt-[78px] sm:mt-[165px] mx-auto mb-12 max-w-[1200px] w-full px-4">
      <div className="py-6">
        <h1 className="text-[30px] text-left font-semibold mb-8 sm:text-[45px]">{blog.title || blog.page_heading || blog.headline}</h1>

        {/* Optional hero image */}
        {blog.image && (
          <div className="relative w-full h-[250px] sm:h-[600px] rounded-[40px] overflow-hidden shadow-[0px_0px_2px_1px_#00000040] mb-8">
            <Image src={blog.image} alt={blog.title || "hero"} fill className="object-cover" priority />
          </div>
        )}
        ...
        {!htmlContent && mdContent && (processing ? <p>Loading...</p> : renderHtml(mdHtmlCache[blogId || ""] || ""))}

        {/* If raw text only */}
        {!htmlContent && !mdContent && (
          <div className="prose prose-lg max-w-none whitespace-pre-line">
            <p>{blog.rawText || blog.text || blog.content || ""}</p>
          </div>
        )}

        {/* Optional FAQ for unstructured blog */}
        {blog.faq && blog.faq.length > 0 && (
          <div className="mt-16 max-w-[1200px] mx-auto">
            <h2 className="text-3xl font-bold mb-8 text-center text-[#000]">Frequently Asked Questions</h2>
            <div className="space-y-4">
              {blog.faq.map((faqItem: any, index: number) => (
                ...
                >
                  ...
                  >
                    <span className="text-lg">{faqItem.question}</span>
                    <svg
                      className={clsx("w-7 h-7 transform transition-transform duration-300", openFaqIndex === index ? "rotate-180 text-[#025C7A]" : "text-[#03475B]")}
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                      xmlns="http://www.w3.org/2000/svg"
